...
import 'package:ebook/src/classes/chapter.dart';
import 'package:ebook/src/writers/epub/epub_writer.dart';
...
String ao3CSS = '''body { background-color: #ffffff;
text-align: justify;
margin: 2%;
...
void test() async {
  ...
  var path = '${dir.path}/test-folder/test.epub';
  var writer = EpubWriter();
  writer.formatter.useStylesheetFile = true;
  // Get full filepath to '.\stylesheet.css'
  String test = dirname(Platform.script.toString()).replaceFirst('file://', '');
  File testFile = File('$test/ao3.css');
  bool exists = testFile.existsSync();
  writer.formatter.stylesheetPath = File('$test/ao3.css').absolute.path;
  writer.book = Book(
    chapters: [
      Chapter(
        title: 'Chapter 1',
  ...
  var bookPath =
      '${dir.path}/test-folder/${writer.book!.metadata!['uid']}.epub';
  writer.write(filePath: bookPath);
}
